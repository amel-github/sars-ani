...
# To visualize fcase atality rate (proportion of individuals who died from SARS-CoV-2 among all individuals diagnosed with the disease), we plot (number of reported deaths / number of reported cases) on the heat map
...
sars_df %>%
  ...
  theme(axis.text.x = element_text(angle=90, hjust=1, vjust = 0.5), 
        axis.title.x = element_text(vjust= -1),
        axis.title.y = element_text(vjust= 3))+
 scale_fill_viridis_c(option = 'D', trans = "sqrt",  breaks = trans_breaks("sqrt", function(x) x ^ 2), name = "Fatality rate", na.value = "firebrick4") +
 guides(fill = guide_colourbar(barwidth = 2, barheight = 10)) +
  theme(text = element_text(size = 24)) 
...
The figures describe the number (and percentages) of **events** (one event may include one or more than one cases).
...
The following bar plots summarize clinical signs of SARS-CoV-2 in some animal species after natural infection. The figures show the **number of events** in which each clinical sign was reported (one event may include one or more than one cases).<br>  
